...
function AddProduct({ onBack }) {
  ...
  const validateForm = () => {
    const { brandName, name, category, price, stockQuantity, description, image } = formData;
    ...
    if(!brandName){
      ...
      toast.error('stockQuantity cannot be empty!');
      return false;
    }else if(!description){
...
const handleSubmit = async (e) => {
  ...
  try {
    const response = await axios.post('http://localhost:8080/api/admin/addproduct',data,{
        headers: {
  ...
  return (
    <div>
      <header className="aheader">
        <h1 className="apage-title">Add New Product</h1>
      </header>
